...
| # | Feature | Description | How It Works |
|---|---------|-------------|--------------|
| 2.1 | **AI Symptom Analysis** | Analyzes the nurse-entered reason and symptom details against the student's medical profile to suggest possible diagnoses. | A POST request to `/api/ai-diagnosis` sends the case to Google Gemini 2.5 Flash with a structured prompt. Gemini returns a guaranteed-structure JSON response using `responseMimeType: application/json` and a schema. |
